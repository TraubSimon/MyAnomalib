"""Anoamlib dataset base class

This module provides base dataset class for Anomalib datasets. The dataset is based on a 
dataframe that contains the information needed by the dataloader to load each dataset 
item into memory.

The samples adataframe mus be set from the subclass using the setter of the 
``samples`` property. 
...
The DataFrame must include at least the following columns:
    ...
    - ``label_index`` (int): Index of the anomaly label, typically 0 for 'normal' and
        1 for 'anomalous'
    - ``mask_path`` (str, optional): Path to the ground truth masks (for 
        anomalous images only). Required if the task is `segmentation`
...
from src.data.dataclasses import DatasetItem, ImageBatch, ImageItem 
from src.data.utils import LabelName, read_image, read_mask

_EXPECTED_COPLUMNS = ["image_path", "split"]
...
class AnomalibDataset(Dataset, ABC):
    ...
    This dataset is designed to work with image-based anomaly detection tasks. It
    supports both classification and segmentation tasks.

    Args:
        transfrom (Transform | None, optional): Transforms to apply to the input images.
            Defaults to ``None``
    Example: 
    ...
    Note: The example abouve is illustrative
    """
    ...
    @property
    def name(self) -> str:
        """Get the na,e of the dataset
        
        Returns:
            str: Name of the dataset derived from the class name
                with 'Dataset' suffix and removed if present
        """
        class_name = self.__class__.__name__ 

        # remove '_dataset' suffix from the class nmae
    ...
    def subsample(self, indices: Sequence[int], inplace: bool = False) -> "AnomalibDataset":
        """Create a subset of the dataset using the provided idices
        
        Args:
            indices (Sequence[int]): Indices at which the dataset is to be subsampled.
            inplace (bool): When true, modify the instance itself.
                Defaults to ``False``.
        ...
        Raises: 
            ValueError: If duplicated indices are provided.
        ...
        if len(set(indices) != len(indices)):
            msg = "No duplicate indices allowed"
    ...
    def samples(self) -> DataFrame:
        ...
        Raises:
            RunTimeError: If samples in DataFrame has not been set.
        """
        if self._samples is None:
            msg = (
                "Dataet does not have a samples dataframe. Ensure that a dataframe"
                "has bee assigned to `dataset.sample`.")
            raise RuntimeError(msg)
        return self._samples 
    
    @samples.setter
    def samples(self, samples: DataFrame) -> None:
        """Set the sampes in DataFrame
        
        Args: 
            sample (DataFrame): DataFrame containing dataset samples.
        ...
        if not all(col in samples.columns for col, in _EXPECTED_COPLUMNS):
            msg = f"samples must have (at least columns) {_EXPECTED_COPLUMNS}, found {samples.columns}"
            raise ValueError(msg)
    ...
    def __getitem__(self, index: int) -> DatasetItem:
        """Get dataset item for a given dataset.
        ...
        if self.task == TaskType.CLASSIFICATION:
            ...
            # Only Anomalous (1) images have a mask in anomaly datasets
            # Therefore create empty mask for Normal (0) images
            mask = (
                Mask(torch.zeros(image.shape[-2:]).to(torch.uin8)
    ...
    def __add__(self, other_dataset: "AnomalibDataset") -> "AnomalibDataset":
        ...
        if not isinstance(other_dataset, self.__class__):
            msg = "Cannot concatenate datasets that are not the same type."
            raise TypeError(msg)
    ...
    def collate_fn(self) -> Callable:
        """Get the collate functrion for batching dataset items.
        
        Returns 
            Callable: Collate functio for ImageBatch
