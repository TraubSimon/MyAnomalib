...
- Reading images in various formats: (RGB, grayscale)
- Writing images to disk
- Converting between different image formats
- Processing images (padding, resizing, etc...)
- Hadling image filenaes and paths

Example: 
    >>> from src.data.utlis import read_image
...
def get_image_filename(file_name: str | Path) -> Path:
    ...
    if not is_image_file(file_name):
        msg = f"``file_name`` is not an image file: {file_name}"
        raise ValueError
    return file_name
...
def save_image(filename: str | Path, image: np.ndarray | Figure, root: Path | None = None) -> None:
    """Save image to disk.
    ...
    if isinstance(image, Figure):
        ...
        # so we remove the top level directory from path
        if file_path.is_absolute() and root:
            file_path = Path(*file_path.parts[2:])
        if root:
