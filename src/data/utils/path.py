...
This moudle provides utilities for:
...
Example: 
    >>> from src.data.utils.path import validate_path
...
def _check_and_convert_path(path: str | Path) -> None:
...
def prepare_files_labels(
    ...
    if not all(extension.startsWith(".") for extension in extensions):
        msg = f"All extension {extensions} must start with the dot."
...
def resolve_path(fold: str | Path, root: str | Path | None = None) -> Path:
    ...
    # path is relative
        ...
        # root provided; preped root and return absolute path
...
def validate_path(
    ...
    """Validate path for existence, permission and extension.
    ...
    Returns: 
        Path: Valudated Path object

    Raises:
        TypeError: If path is invalid type
        ValueError: IF path is too long ot has invalid characters/extensions
        FileNotFoundError: If path dosen't exist when required
        Permission Error: If path lacks required permissions
    ...
    # In case path ``should_exist``, the path is valud and should be 
...
def validate_and_resolve_path(
    ...
    """Validate and resolve path by combining validation and reolution.
    
    Args: 
        folder: Folder location containing image or mask data
        root: Root directory for the dataset
        base_dir: Base diretory to restrict file access
