...
- This test set lacks normal images
- The datasets needs a validation set
- Class balance needs ot be maintained during splits

Example:
    >>> form src.data.utils.split import random_split
    >>> train_set, val_set = random_split(dataset, split_ratio= 0.2)
    >>> len(train_set), len(val_set)
    800, 200

    >>> # label aware splitting preservig class distributions
...
if TYPE_CHECKING:
    from src.data import datasets as data 
...
def concatenate_datasets(
    ...
    Args: 
        dataset: Sequence of at least two datasets to concatenate
    
    Returns: 
        Combined dataset conatining samples from all input datasets
...
def random_split(
    ...
    """Randomly splits a dataset into multiple subsets.
    Args:
    ...
    if not (math.isclose(sum(split_ratio), 1) and sum(split_ratio <= 1)):
        msg = f"All split ratios must sum up to 1, found [{sum(split_ratio)}]"
        raise ValueError(msg)
    if not all(0 < ratio < 1 for ratio in split_ratio):
        msg = f"All split rations must be between 0 and 1, found {split_ratio}"
        raise ValueError(msg)
    ...
    for label_dataset in per_label_datasets:
        ...
        if 0 in subset_lengths:
            msg = "Zero subset leght encoutered during splitting. This means on of your subsets" 
            "might be empty or devoid of either normal or anomalous images."
            logger.warning(msg)
         
        # perform random subsampling
        random_stat = torch.Generator().manual_seed(seed) if seed else None
        indices = torch.randomperm(len(label_dataset.samples), generator=random_stat)
        subsets.append(
