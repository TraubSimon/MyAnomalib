"""EfficientAD: Accurate visual Anomaly Detection at Millisecond-Level Latncies.
...
from src.components import AnomalibModule
...
from src.efficient_ad.torch_model import EfficientAdModel, reduce_tensor_elems, EfficientAdModelSize
...
class EfficientAD(AnomalibModule):
    """PL Lightning Module for EfficientAD algorithm.
        
    The EfficientAd model usess a student-teacher architecture with a pretrained
    ...
    Args:
        ...
        padding (bool): Use padding in convoltional layers.
            Defaults to ``False``
        pad_maps (bool): Relevant if ``padding=False``. If ``True``, pads the anomaly 
            maps to match the size of ``padding=True`` case 
            Defaults to ``True``
        pre_processor (PreProcessor | bool, optional): Preprocessor used to transform 
    ...
    def on_train_start(self) -> None:
        ...
        1. Validate training paramters (batch_size=1, no normalization)
        2. Sets up pre-trained teacher model
        3. Prepares ImageNette dataset
        4. Calculates channel statistics

        Raises: 
            ValueError: If ``train_bact_size != 1`` or transforms contain
                normalization.
        """
        ...
        if self.pre_processor and extract_transform_by_type(self.pre_processor.transform, Normalize):
