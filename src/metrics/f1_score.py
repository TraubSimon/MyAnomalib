...
This module provides two metrics for evaluating binary classification perfomance:

- ``F1Score``: Standard F1 score metric that compute the harmonic mean og 
...
Example:
    >>> from anomalib.metric import F1Score, F1Max
    >>> import torch
    >>> # Create sample data
    >>> preds = torch.tensor([0,1, 0.4, 0.35, 0.8])
    >>> targed = torch.tensor([0, 0, 1, 1])
...
from src.metrics.base import AnomalibMetric
from .precision_recall_curve import BinaryPrecisionRecallCurve

class F1Score(AnomalibMetric, BinaryF1Score):
    """Wrapper to add AnomalibMatric functionality to F1Score metric.
    
    This class wraps the ``torchmetrics.BinayF1Score`` to make it compatible
