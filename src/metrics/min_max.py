...
This moudle provides the ``MinMax`` metric class which tracks the minimum and
maximum values seen accross batches of data. This is useful for normalizing 
predictions or monitoring value ranges during training. 

Example:
    >>> from src.metrics import MinMax
    >>> import torch
    ...
    >>> new_predicitons = torch.tensor([0.3251, 0.3169, 0.3072, 0.6247, 0.9999])
...
class _MinMax(Metric):
    ...
    This metric maintains running minium and maximum values across all batches
    it processes. It is ueseull for tasks like nomalization or monitoring the 
    range of values during training.

    Args:
        full_state_update (bool, optional): Wheter to update the internal state 
    ...
    Example:
        >>> from src.metrics.min_max import _MinMax
        >>> import torch
        >>> # Create metric
        >>> minmax = _MinMax()
        >>> # Update with batches
