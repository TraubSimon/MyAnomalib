...
metric that does not apply sigmoid normalization to predction thrsholds, 
unlike the standard torchmetrics implementation.

See https://github.com/Lightning-AI/torchmetrics/issues/1526

Example:
    >>> import torch
    >>> from src.metrics import BinaryPrecisionRecallCurve
...
class BinaryPrecisionRecallCurve(_BinaryPrecisionRecallCurve):
    ...
    @staticmethod
    def _binary_precission_recall_curve_format(
        preds: Tensor, 
        target: Tensor, 
        ...
        Args:
            ...
            threshold (int | list[float] | Tensor | None): Thresholds
    ...
    def update(self, preds: Tensor, target: Tensor) -> None:
        ...
        Unlike the base class, this method accepts raw predictions
        without applying sigmoid normalization.
        
        Args:
            preds (Tensor): Raw predicted scores or probabilities
            targte (Tensor): Ground truth binary label (0 ro 1)
        """
        preds, target, _ = BinaryPrecisionRecallCurve._binary_precission_recall_curve_format(
            preds, 
            target, 
            self.thresholds, 
            self.ignore_index,
