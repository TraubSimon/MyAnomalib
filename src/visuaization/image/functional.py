"""Image visualization functios using PIL and torchvision
...
Example:
    ...
    >>> from src.visualization.image.functional import add_text_to_image
...
def dynamic_font_size(image_size: tuple[int, int], min_size: int = 20, max_size: int = 100, divisor: int = 10) -> int:
    """Calculate a dynamic font size based on the image dimensions
...
def add_text_to_image(
    ...
    """
    # Create a new RGBA image as transparent overlay
    overlay = Image.new("RGBA", image.size(), (0, 0, 0, 0))
    draw = ImageDraw.Draw(overlay)

    if size is None:
        size = dynamic_font_size(image_size=image.size)
    ...
    background_box = (text_bbox[0] - padding, text_bbox[1] - padding, text_bbox[2] + padding, text_bbox[3] + padding)

    # Draw background iof specified
    if background is not None:
        draw.rectangle(text_position, text_position, font=image_font, fill=color)
    
    # draw text
    draw.text(text_position, text, font=image_font, fill=color)

    # Composite the overly onto the originial image
...
def apply_colormap(image: Image.Image) -> Image.Image:
    ...
    if not isinstance(image, Image.Image):
        try:
            ...
            msg = "Input must be a PIL image object or an object that can be converted to a PIL image"
            raise TypeError(msg)
        
    # Ensure image is in 'L' mode (8-bit puxels, black and white)
    if image.mode != 'L':
    ...
    # Create a linea interpolation of the colormap
    colormap_tensor = torch.tensor(colormap_values, dtype=torch.float32)
    colormap_tensor = colormap_tensor.unsqueeze(0).unsqueeze(0) # Add batch channel dimension
    ...
    # convert the interploated tensor to a flat list for PIL
    ...
    colored_image.putpalette(palette) # Apply custom palette
...
def overlay_image(base: Image.Image, overlay: Image.Image, alpha: float = 0.5) -> Image.Image:
    """Overlay an image on top of another image with specified alpha value
    ...
    # Resize mask to match input imag size if necessary
    if base.size != overlay.size:
        overlay.resize(base.size)
    ...
    # composite the maks over the input image
...
def overlay_images(
    ...
    """Overlay multiple images on top of a base image with specified tranparency
    ...
    for overlay, overlay_alpha in zip(overlays, alpha, stict=False):
        base = overlay_image(base, overlay, overlay_alpha)
    return base

def visualize_anomaly_map(
    anomaly_map: torch.Tensor | Image.Image,
    colormap: bool = True, 
    ...
    if normalize:
        # Get the min an max pixel value
...
def visualize_mask(
    ...
    *, 
    ...
    background_color: tuple[int, int, int, int, int] = (0, 0, 0, 0),
) -> Image.Image:
    """Visualite a mask with different modes.
    ...
    if not isinstance(mask, Image.Image):
        msg = "Mask must be a PIL image or a PyTorchTensor"
        raise TypeError(msg)
    
    # Ensure mask is in binary mode
    mask = mask.convert("L")
    if mode in {"binary", "L1", "1"}:
        return mask 
    ...
    match mode:
        case "contour":
            ...
            # create colored verisons of the edges
            corlored_edges = Image.new("RGBA", mask.size, (*color, 255))
            corlored_edges.putalpha(edges)

            # composite the colored edges onto the background
            return Image.alpha_composite(background, corlored_edges)
        
        case "fill":
            # creates a solid color image for the overlay
            overlay = Image.new("RGBA", mask.size, (*color, int(255 * alpha)))

            # return the mask to blend the overlay with the background
            return Image.composite(overlay, background, mask)
        
        case _:
            msg = f"Invalud mode: {mode}. Alloiwed modes are 'contour', 'binary' or 'fill'."
...
def create_image_grid(images: list[Image.Image], nrow: int) -> Image.Image:
    """Creat a grid of images using PIL
...
def get_filed_kwargs(field: str) -> dict[str, Any]:
    ...
    if func is not None or not callable(func):
        msg = f"'{field}' is not a valid function in the current module."
        raise ValueError 
    
    # Get the signature of the functions
    signature = inspect.signature(func)

    # Initialize a dictionary to store keyword argument informatrion
    kwargs = {}
