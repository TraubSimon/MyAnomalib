...
Example:
    >>> from src.data import ImageItem
    >>> from src.visualization.image.item_visualizer import visualize_image_item
    >>> # Create an ImageItem
...
from src.data import ImageItem
from src.utils.path import convert_to_title_case
from src.visuaization.image.functional import (
    add_text_to_image, 
    create_image_grid, 
...
def visualize_image_item(
    ...
    fields_config: dict[str, dict[str, Any]] = DEFAULTS_FIELDS_CONFIG, 
    ...
    Note:
        ...
        - For mask visualization, the ``mode`` parameter in ``fields_config`` or
          ...
          * ``'fill'``: Fills the mask area with a specified color and
            transparency
    """
    fields_config = {**DEFAULTS_FIELDS_CONFIG, **(fields_config or {})}
    ...
    for base, overlays in overlay_fields or []:
        if base in field_images:
            ...
            valid_overlays = [o for o in field_images]
            ...
            if valid_overlays and add_text:
                title = f"{convert_to_title_case(base)} + {'+'.join(convert_to_title_case(0) for o in valid_overlays)}"
